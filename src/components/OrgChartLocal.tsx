...
import { Download, Settings as SettingsIcon, Users, Copy, Check, ArrowLeft } from 'lucide-react';
...
export default function OrgChartLocal({ onBack }: OrgChartLocalProps) {
  ...
  const canvasSize: CanvasSize = (settings?.canvasSize as CanvasSize) || 'auto';
  ...
  const loadData = () => {
    ...
    if (savedLevels && savedEmployees) {
      const levels: Level[] = JSON.parse(savedLevels);
      const rawEmployees = JSON.parse(savedEmployees);

      const levelMap = new Map(levels.map((l, i) => [l.level_name.toLowerCase(), { color: l.color, order: i + 1 }]));

      const employeesWithIds = rawEmployees.map((emp: any, index: number) => {
        const levelInfo = levelMap.get(emp.level_name.toLowerCase()) || { color: '#3b82f6', order: 999 };
        return {
          id: `emp-${index}`,
      ...
      const nameToIdMap = new Map(employeesWithIds.map((e: Employee) => [e.employee_name.toLowerCase(), e.id]));

      const finalEmployees = employeesWithIds.map((emp: any) => ({
        ...emp,
        manager_id: emp.manager_name ? nameToIdMap.get(emp.manager_name.toLowerCase()) : null,
      }));
  ...
  const buildTree = (): TreeNode[] => {
    ...
    employees.forEach(emp => {
      ...
      if ((emp as any).manager_id) {
        const parent = employeeMap.get((emp as any).manager_id);
        if (parent) {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  {settings?.companyName || 'Organization Chart'}
                </h1>
